...
class _DataLoaderWithNext(torch.utils.data.DataLoader):
    """
    PyTorch's multiprocessing DataLoader iterator on Windows does not expose
    a `.next()` method, yet the provided unit tests rely on it.
    This thin wrapper makes sure every iterator returned exposes both
    `__next__` and `next`, regardless of the underlying worker configuration.
    """
...
def get_data_loaders(
    ...
    data_loaders["train"] = _DataLoaderWithNext(
        train_data,
    ...
    data_loaders["valid"] = _DataLoaderWithNext(
        valid_data,
        batch_size=batch_size,
    ...
    data_loaders["test"] = _DataLoaderWithNext(
        test_data,
        batch_size=batch_size,
